          ...
          <div className="flex gap-10">
            ...
            <div className="flex flex-col gap-3 items-start justify-start text-muted-foreground">
              {profileData.map(({ icon, label, value, isNested }, idx) => (
                <p key={idx} className="flex flex-row gap-2 items-center">
                  {icon}
                  {label}
                  {isNested ? (
                    <span className="ml-4 text-white">{value}</span>
                  ) : (
                    <p className="ml-2 text-white">{value}</p>
                  )}
                </p>
              ))}
            </div>
